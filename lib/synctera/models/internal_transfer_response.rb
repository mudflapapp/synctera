  ...
  class InternalTransferResponse
    ...
    def self.openapi_types
      {
        ...
        :'type' => :'String',
        :'id' => :'String',
        :'status' => :'InternalTransferResponseStatus'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::InternalTransferResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::InternalTransferResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def capture_mode=(capture_mode)
      ...
      unless validator.valid?(capture_mode)
        warn "invalid value for \"capture_mode\", must be one of #{validator.allowable_values}."
      end
      @capture_mode = capture_mode
    end
    ...
    def hash
      [amount, capture_mode, currency, expires_at, final_customer_id, memo, metadata, originating_account_alias, originating_account_customer_id, originating_account_id, receiving_account_alias, receiving_account_customer_id, receiving_account_id, reference_id, tenant, type, id, status].hash
    end
