  ...
  class PatchAccountLineOfCredit
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::PatchAccountLineOfCredit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::PatchAccountLineOfCredit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [access_status, account_number, account_number_masked, account_purpose, account_type, application_id, balances, bank_routing, creation_time, currency, customer_ids, customer_type, exchange_rate_type, iban, id, is_account_pool, is_sar_enabled, last_updated_time, metadata, nickname, status, swift_code, credit_limit, grace_period, is_ach_enabled, is_eft_ca_enabled, is_p2p_enabled, is_wire_enabled, minimum_payment, note].hash
    end
