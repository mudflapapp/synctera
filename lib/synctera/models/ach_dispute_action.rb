...
module Synctera
  ...
  class AchDisputeAction
    attr_accessor :action

    # Message text related to ACH dispute action * Max length for debit transactions is 38 characters 
    attr_accessor :message

    # This field is required for the STOP_PAYMENT dispute action type. When the associated dispute is accepted any incoming ACH transactions where the sender name matches originator_name will be ignored. 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::AchDisputeAction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::AchDisputeAction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@originator_name.nil? && @originator_name.to_s.length > 100
        invalid_properties.push('invalid value for "originator_name", the character length must be smaller than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if !@originator_name.nil? && @originator_name.to_s.length > 100
    ...
    # Custom attribute writer method with validation
    # @param [Object] originator_name Value to be assigned
    def originator_name=(originator_name)
      if originator_name.nil?
        fail ArgumentError, 'originator_name cannot be nil'
      end

      if originator_name.to_s.length > 100
        fail ArgumentError, 'invalid value for "originator_name", the character length must be smaller than or equal to 100.'
      end
    ...
    def hash
      [action, message, originator_name, payment_rail, reason_code, return_code, state, supporting_doc_id].hash
    end
