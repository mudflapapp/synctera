  ...
  class RiskRating
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::RiskRating` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::RiskRating`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'next_review')
        self.next_review = attributes[:'next_review']
      end
      ...
      if attributes.key?(:'risk_review')
        self.risk_review = attributes[:'risk_review']
      end
    ...
    def valid?
      ...
      risk_level_validator = EnumAttributeValidator.new('String', ["high", "low", "medium"])
    ...
    def hash
      [configuration_id, id, next_review, risk_level, risk_review, risk_score].hash
    end
