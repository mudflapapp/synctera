  ...
  class FleetsEmv
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::FleetsEmv` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::FleetsEmv`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'trailer_number')
        self.trailer_number = attributes[:'trailer_number']
      end
    ...
    def hash
      [employee_number, expanded_fuel_type, fuel_gross_amount, fuel_net_amount, fuel_quantity, fuel_unit_of_measure, fuel_unit_price, non_fuel_gross_amount, non_fuel_item_details, non_fuel_net_amount, odometer_reading, service_type, trailer_number, type_of_purchase, vat_tax_rate].hash
    end
