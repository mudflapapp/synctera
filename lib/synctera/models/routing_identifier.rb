...
module Synctera
  # An object that holds a routing number and its corresponding information
  class RoutingIdentifier
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::RoutingIdentifier` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::RoutingIdentifier`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'payment_rails')
        if (value = attributes[:'payment_rails']).is_a?(Array)
          self.payment_rails = value
        end
      end
    ...
    def hash
      [location, payment_rails, routing_number].hash
    end
