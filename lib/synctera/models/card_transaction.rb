  ...
  class CardTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::CardTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::CardTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @card_transaction.nil?
        invalid_properties.push('invalid value for "card_transaction", card_transaction cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @card_transaction.nil?
    ...
    def hash
      [account_id, amount, creation_time, currency, customer_id, dc_sign, decline, description, effective_date, enhanced_transaction, force_post, fraud_risk, group_id, hold_expiration_time, id, internal_account_id, last_updated_time, metadata, partial_hold, pending_amount, posted_amount, posted_date, reference_id, status, system_description, transaction_time, type, card_transaction, subtype].hash
    end
