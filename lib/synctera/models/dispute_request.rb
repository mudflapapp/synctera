...
module Synctera
  class DisputeRequest
    ...
    # Memo or note describing the dispute.
    attr_accessor :memo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::DisputeRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::DisputeRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@memo.nil? && @memo.to_s.length > 100
        invalid_properties.push('invalid value for "memo", the character length must be smaller than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if !@memo.nil? && @memo.to_s.length > 100
    ...
    def memo=(memo)
      ...
      if memo.to_s.length > 100
        fail ArgumentError, 'invalid value for "memo", the character length must be smaller than or equal to 100.'
      end
    ...
    def hash
      [date_customer_reported, disputed_amount, memo, transaction_id].hash
    end
