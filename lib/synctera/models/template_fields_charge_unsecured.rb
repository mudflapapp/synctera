...
module Synctera
  # For creating CHARGE_UNSECURED accounts 
  class TemplateFieldsChargeUnsecured
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::TemplateFieldsChargeUnsecured` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::TemplateFieldsChargeUnsecured`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account_type, bank_account_id, bank_country, currency, grace_period, minimum_payment, spend_control_ids].hash
    end
