...
module Synctera
  # Account representing a general ledger (vs a customer account).  In production, general ledger accounts can only be created or updated by a Synctera administrator.  General ledger accounts are in Alpha status, and cannot yet be created. We may make breaking changes. 
  class AccountGeneralLedger
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::AccountGeneralLedger` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::AccountGeneralLedger`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
