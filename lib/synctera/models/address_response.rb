  ...
  class AddressResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::AddressResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::AddressResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      address_type_validator = EnumAttributeValidator.new('String', ["BILLING", "LEGAL", "OTHER", "SHIPPING"])
    ...
    def address_type=(address_type)
      validator = EnumAttributeValidator.new('String', ["BILLING", "LEGAL", "OTHER", "SHIPPING"])
      unless validator.valid?(address_type)
        warn "invalid value for \"address_type\", must be one of #{validator.allowable_values}."
      end
      @address_type = address_type
    end
    ...
    def hash
      [business_id, creation_date, customer_id, is_active, last_updated_time, address_line_1, address_line_2, address_type, city, country_code, id, nickname, postal_code, state].hash
    end
