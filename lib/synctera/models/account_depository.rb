  ...
  class AccountDepository
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::AccountDepository` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::AccountDepository`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [access_status, account_number, account_number_masked, account_purpose, account_type, application_id, balances, bank_routing, creation_time, currency, customer_ids, customer_type, exchange_rate_type, iban, id, is_account_pool, is_sar_enabled, last_updated_time, metadata, nickname, status, swift_code, balance_ceiling, balance_floor, fee_product_ids, interest_product_id, note, overdraft_limit, spend_control_ids, spending_limits].hash
    end
