...
module Synctera
  # Rails available to enable on charged unsecured accounts
  class RailsChargeUnsecured
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::RailsChargeUnsecured` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::RailsChargeUnsecured`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [is_ach_enabled, is_card_enabled, is_eft_ca_enabled, is_p2p_enabled, is_synctera_pay_enabled, is_wire_enabled].hash
    end
