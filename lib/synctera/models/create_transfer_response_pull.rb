...
module Synctera
  class CreateTransferResponsePull
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::CreateTransferResponsePull` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::CreateTransferResponsePull`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account_id, amount, creation_time, currency, customer_id, external_card_id, id, last_updated_time, merchant, network_decline_details, reason, status, tenant, transaction_id, three_ds_id, type].hash
    end
