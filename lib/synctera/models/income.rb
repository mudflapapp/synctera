  ...
  class Income
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::Income` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::Income`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      source_validator = EnumAttributeValidator.new('String', ["ALIMONY", "EMPLOYMENT", "INHERITANCE", "INVESTMENT", "OTHER", "PENSION", "REAL_ESTATE", "RETIREMENT_INCOME", "SCHOLARSHIP", "SOCIAL_SECURITY", "UNEMPLOYMENT"])
    ...
    def source=(source)
      validator = EnumAttributeValidator.new('String', ["ALIMONY", "EMPLOYMENT", "INHERITANCE", "INVESTMENT", "OTHER", "PENSION", "REAL_ESTATE", "RETIREMENT_INCOME", "SCHOLARSHIP", "SOCIAL_SECURITY", "UNEMPLOYMENT"])
      unless validator.valid?(source)
        warn "invalid value for \"source\", must be one of #{validator.allowable_values}."
      end
      @source = source
    end
    ...
    def hash
      [amount, currency, frequency, source].hash
    end
