  ...
  class PaymentSchedule
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::PaymentSchedule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::PaymentSchedule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @payment_instruction.nil?
        invalid_properties.push('invalid value for "payment_instruction", payment_instruction cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @payment_instruction.nil?
    ...
    def hash
      [description, id, metadata, next_payment_date, payment_instruction, schedule, status].hash
    end
