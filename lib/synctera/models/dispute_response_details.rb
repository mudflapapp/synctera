  ...
  class DisputeResponseDetails
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::DisputeResponseDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::DisputeResponseDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account_id, applicable_regulation, creation_time, credit_status, currency, customer_id, date_customer_reported, decision, dispute_documents, disputed_amount, id, last_action_by, last_updated_time, network, payment_rail, status, tenant, timestamp_provisional_credit_due, transaction_id].hash
    end
