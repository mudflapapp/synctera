  ...
  class Lookup3dsRequestBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::Lookup3dsRequestBase` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::Lookup3dsRequestBase`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def authentication_indicator=(authentication_indicator)
      ...
      unless validator.valid?(authentication_indicator)
        warn "invalid value for \"authentication_indicator\", must be one of #{validator.allowable_values}."
      end
      @authentication_indicator = authentication_indicator
    end
    ...
    def device_channel=(device_channel)
      ...
      unless validator.valid?(device_channel)
        warn "invalid value for \"device_channel\", must be one of #{validator.allowable_values}."
      end
      @device_channel = device_channel
    end
    ...
    def transaction_mode=(transaction_mode)
      ...
      unless validator.valid?(transaction_mode)
        warn "invalid value for \"transaction_mode\", must be one of #{validator.allowable_values}."
      end
      @transaction_mode = transaction_mode
    end
    ...
    def hash
      [authentication_indicator, device_channel, id, transaction_mode].hash
    end
