  ...
  class PhysicalCardResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Synctera::PhysicalCardResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Synctera::PhysicalCardResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def reissue_reason=(reissue_reason)
      ...
      unless validator.valid?(reissue_reason)
        warn "invalid value for \"reissue_reason\", must be one of #{validator.allowable_values}."
      end
      @reissue_reason = reissue_reason
    end
    ...
    def hash
      [account_id, business_id, card_product_id, creation_time, customer_id, emboss_name, expiration_month, expiration_time, expiration_year, id, is_pin_set, last_four, last_modified_time, metadata, reissue_reason, reissued_from_id, reissued_to_id, timestamp_pin_set, type, form, card_image_id, shipping, card_status, memo, pending_reasons, status_reason, card_fulfillment_status, fulfillment_details, tracking_number, bin, card_brand, physical_card_format, tenant].hash
    end
