...
module Synctera
  class DocumentsApi
    ...
    # @option opts [Encryption] :encryption  (default to 'NOT_REQUIRED')
    ...
    # @param document_id [String] The unique identifier of the document.
    # @param patch_document [PatchDocument] 
    # @param [Hash] opts the optional parameters
    # @return [Document]
    def update_document(document_id, patch_document, opts = {})
      data, _status_code, _headers = update_document_with_http_info(document_id, patch_document, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @return [Array<(Document, Integer, Hash)>] Document data, response status code and response headers
    def update_document_with_http_info(document_id, patch_document, opts = {})
      if @api_client.config.debugging
      ...
      if @api_client.config.client_side_validation && patch_document.nil?
        fail ArgumentError, "Missing the required parameter 'patch_document' when calling DocumentsApi.update_document"
      end
      # resource path
      local_var_path = '/documents/{document_id}'.sub('{' + 'document_id' + '}', CGI.escape(document_id.to_s))
